...
// TestUpdateAlertContactHandler_RefusesToUnlinkWebhook pins that a PUT which
// omits id_webhook is rejected instead of clearing the stored value.
// UpdateAlertContact writes through Updates(map[...]), which persists zero
// values, so clearing it would detach the contact from its webhook and stop
// SendAllValidatorAlerts' id_webhook join from ever matching it again.
func TestUpdateAlertContactHandler_RefusesToUnlinkWebhook(t *testing.T) {
	db := testoutils.NewTestDB(t)
	internal.Config.DevMode = true
	defer func() { internal.Config.DevMode = false }()
	...
	if err != nil || len(contacts) != 1 {
		t.Fatalf("list contacts: %v (n=%d)", err, len(contacts))
	}
	contactID := contacts[0].ID

	// id_webhook omitted entirely -> decodes to 0 -> would blank the link.
	body := fmt.Sprintf(`{"id":%d,"moniker":"val1","namecontact":"On-call","mention_tag":"123456789012345678"}`, contactID)
	req := httptest.NewRequest(http.MethodPut, "/alert-contacts", bytes.NewBufferString(body))
	...
	after, err := database.GetAlertContact(db, contactID, userID)
	if err != nil || after == nil {
		t.Fatalf("reload contact: %v (nil=%t)", err, after == nil)
	}
	if after.IDwebhook != webhookID {
		t.Fatalf("id_webhook = %d, want %d (link was cleared)", after.IDwebhook, webhookID)
	}
}

// TestDeleteMonitoringWebhookHandler_IDZeroDoesNotSweepUnlinkedContacts pins
// that the cascade can't be abused via id=0. Webhook IDs are autoIncrement so
// 0 never identifies a real webhook, but id_webhook=0 is the legitimate state
// of every contact created without a webhook link — matching it would delete
// all of them while reporting success.
func TestDeleteMonitoringWebhookHandler_IDZeroDoesNotSweepUnlinkedContacts(t *testing.T) {
	db := testoutils.NewTestDB(t)
	internal.Config.DevMode = true
	defer func() { internal.Config.DevMode = false }()

	userID := "user-a"
	if err := database.InsertAlertContact(db, userID, "val1", "Unlinked", "123456789012345678", database.NoWebhookLinked); err != nil {
		t.Fatalf("seed unlinked alert contact: %v", err)
	}

	req := httptest.NewRequest(http.MethodDelete, "/webhooks/validator?id=0", nil)
	req.Header.Set("X-Debug-UserID", userID)
	rec := httptest.NewRecorder()
	...
	if len(contactsAfter) != 1 {
		t.Fatalf("contacts after delete = %d, want 1 (unlinked contact was swept up)", len(contactsAfter))
	}
}
