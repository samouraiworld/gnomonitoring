...
func TestInsertAndGetUser(t *testing.T) {
	...
	err := internal.InsertUser("user123", "test@example.com", "Alice", db)
	if err != nil {
		t.Fatalf("InsertUser failed: %v", err)
	}

	user, err := internal.GetUserById(db, "user123")
	if err != nil {
...
func TestInsertWebhookAndList(t *testing.T) {
	...
	webhooks, err := internal.ListWebhooks(db, "user123")
	if err != nil {
...
func TestUpdateUser(t *testing.T) {
	...
	err := internal.InsertUser("user123", "initial@example.com", "Initial", db)
	if err != nil {
		t.Fatalf("InsertUser failed: %v", err)
	}

	err = internal.UpdateUser(db, "UpdatedName", "new@example.com", "user123")
	if err != nil {
