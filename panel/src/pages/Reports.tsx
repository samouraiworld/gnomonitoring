...
export default function Reports() {
  ...
  const handleExportCsv = () => {
    ...
    for (const per of PERIOD_ORDER) {
      headers.push(`${per}_score`, `${per}_tier`, `${per}_sign`, `${per}_vp`, `${per}_proposer`, `${per}_critical`, `${per}_warning`, `${per}_freq`, `${per}_freq_rate`, `${per}_downtime`, `${per}_missed`)
    }
    const lines = sorted.map(r => {
      ...
      for (const per of PERIOD_ORDER) {
        ...
        if (p) {
          cells.push(String(p.score), p.tier, p.sign_rate.toFixed(1), String(p.voting_power), p.proposer_reliability != null ? p.proposer_reliability.toFixed(1) : '', String(p.critical_count), String(p.warning_count), String(p.incident_count), p.incident_rate_per_week.toFixed(2), String(p.downtime_blocks), String(p.missed_blocks))
        } else {
          cells.push('', '', '', '', '', '', '', '', '', '', '')
        }
      }
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ cursor: 'pointer' }} onClick={() => handleSort('freq')}><HeaderTip align="right" tip="Distinct incidents this period (raw count, with the incidents/week-equivalent rate used for scoring in parentheses) — WARNING/CRITICAL alerts collapsed across resends, separated only by a RESOLVED. A better flapping signal than the raw Critical/Warning counts.">Freq{sortIndicator('freq')}</HeaderTip></th>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={r.addr}>
                    ...
                    <td>{p ? `${p.incident_count} (${p.incident_rate_per_week.toFixed(2)}/wk)` : '—'}</td>
